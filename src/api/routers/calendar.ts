...
/**
 * DELETE: Eliminar un calendari (només si és de l'usuari o si és admin)
 * URL: /api/v1/calendar/:id
 */
router.delete('/:id', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!isOwner && !isAdmin) {
            return res.status(403).json({ message: 'No tens permís per eliminar aquest calendari' });
        }

        await sql`DELETE FROM calendar WHERE id = ${id}`;
        res.json({ message: 'Calendari eliminat correctament' });
    } catch (error: any) {
        res.status(500).json({ message: 'Error al eliminar el calendari' });
    }
});
